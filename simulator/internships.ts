...
function completeInternship(state: GameState): void {
  ...
  state.internshipValue = Math.max(state.internshipValue, active.value);
...
function desiredInternshipTier(state: GameState): InternshipTier | undefined {
  if (state.internshipValue < INTERNSHIP_THRESHOLDS.ordinary.value && isEligibleForTier(state, "ordinary")) {
    return "ordinary";
  }
  if (state.internshipValue < INTERNSHIP_THRESHOLDS.strong.value && isEligibleForTier(state, "strong")) {
    return "strong";
  }
  if (state.internshipValue < INTERNSHIP_THRESHOLDS.namedFirm.value && isEligibleForTier(state, "named_firm")) {
    return "named_firm";
  }
  return undefined;
}
...
function isEligibleForTier(state: GameState, tier: InternshipTier): boolean {
  ...
  return (
    ...
    state.internshipValue >= application.priorValueRequired &&
