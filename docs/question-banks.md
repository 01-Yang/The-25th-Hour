...
- 大学课程题全部结束后，只显示总结果、分数或对应修正。
- 升学专业题和行测题全部结束后，不直接向玩家显示成绩、答对题数或是否过线，只记录内部结果，留到最终结局结算时统一揭示。
- 升学专业题内部折算分固定为：答对 `10 / 9 / 8 / 7 / 6` 题，分别折算为 `410 / 395 / 380 / 365 / 350` 分；答对 `5` 题及以下视为 `350` 分以下。
- 行测题内部折算分固定为：答对 `10 / 9 / 8 / 7 / 6` 题，分别折算为 `150 / 145 / 140 / 135 / 130` 分；答对 `5` 题及以下视为 `130` 分以下。
- 雅思考试每年可考 1 次。
- 雅思题库总量为 30 道英文单选题。
- 每次雅思考试从 30 道题中抽取 15 道作答。
- 保研复试与考研初试共用升学专业题库：当前题库 40 道，每次抽 10 道，固定为 `6` 道简单题 + `4` 道困难题。
- 考公、考编、选调共用行测题库：题库 30 道，每次抽 10 道，固定为 `6` 道简单题 + `4` 道困难题。
