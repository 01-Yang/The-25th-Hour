...
6. `technical-architecture.md`：技术栈、存档、排行榜、部署和工程边界。
...
- `technical-architecture.md`：技术架构决策源，不定义玩法规则或内容。
- `development-freeze.md`：旧冻结文档归档入口；冻结口径已并入 `PRD.md`。
- `adr/`：关键结构决策记录。
- `ending-hyperframes-workflow.md`：结局动画工作流，当前不参与游戏系统架构口径。
...
- `numbers.md` 写“数值和结算”。
- `technical-architecture.md` 写“技术栈、存档、排行榜、部署和工程边界”。
